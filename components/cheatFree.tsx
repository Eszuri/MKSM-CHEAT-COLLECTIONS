"use client"
import dragon_logo from "@/public/mksm-dragon-logo.png"
import dataFree from "@/data/dataFree";
import Image from "next/image";
import {Tab, TabList, TabPanel, Tabs} from "react-tabs";
import CheatCodeFree from "@/data/pnachFree";
...
import {Inter, Sriracha, Ysabeau_SC, Galdeano, JetBrains_Mono, Tauri} from "next/font/google"
...
const Ysabeau_SCFont = Ysabeau_SC({
    subsets: ['latin'],
    weight: ['500', '700'],
});

const GaldeanoFont = Galdeano({
    subsets: ['latin'],
    weight: ["400"],
});
...
const CheatPageFree = ({position}: CheatPageFreeType) => {
    const {deskripsi, title, cheatList, listModifier, warning, dateAdd, ytvideo} = dataFree[position];
            ...
            <div className="mt-2 grid gap-5 w-80">
                {cheatList.map((cheat, index) => (
                    <div className="h-12 mt-2 flex gap-2 items-center" key={index}>
                        <Image src={dragon_logo} alt="dragonLogo" width={512} height={512} priority={true} className="w-6 max-w-6 min-w-6 h-6 mt-1" />
                        <div className='grid' >
                            <span className={`${Ysabeau_SCFont.className} font-bold text-emerald-400 text-xl`}>{cheat.li_title}</span>
                            <p className={`${GaldeanoFont.className} text-sm opacity-70 tracking-wider font-Taylor`} >{cheat.li_Desc}</p>
                        </div >
                    </div>
                ))}
            </div >
            ...
            <Tabs className={"mt-20"}>
                ...
                <TabPanel selectedClassName="relative w-full border-2 border-white/40 rounded bg-slate-600/30 p-2">
                    <div className='cursor-pointer w-56 h-9 bg-slate-600 flex justify-center gap-3 items-center rounded hover:bg-slate-700 mb-2'
                        onClick={() => {CopyText(CheatCodeFree[position].raw)}}>
                        <CopyIcon />
                        <span>Copy Text</span>
                    </div>
                    <pre className={`overflow-auto text-nowrap ${JetBrainFont.className} selection:bg-emerald-700`}>{CheatCodeFree[position].raw}</pre>
                </TabPanel>
                <TabPanel selectedClassName="relative w-full border-2 border-white/40 rounded bg-slate-600/30 p-2">
                    <div className='cursor-pointer w-56 h-9 bg-slate-600 flex justify-center gap-3 items-center rounded hover:bg-slate-700 mb-2'
                        onClick={() => {CopyText(CheatCodeFree[position].pnach)}}>
                        <CopyIcon />
                        <span>Copy Text</span>
                    </div>
                    <pre className={`overflow-auto text-nowrap selection:bg-emerald-700 ${JetBrainFont.className}`}>{CheatCodeFree[position].pnach}</pre>
                </TabPanel>
            </Tabs >
        )
    };
    ...
    // list modifier
    function Four() {
        return (<>{listModifier.map((item, index) => (
            <div className={`${item.content === '' ? 'hidden' : 'mt-20'}`} key={index} >
                <h1 className={`text-2xl font-bold uppercase  mb-3 ${TauriFont.className}`}>{item.title}</h1>
                <div className={`bg-slate-700/50 rounded w-full h-full border-white/30 border-2 p-1`}>
                    <pre className={`overflow-auto text-nowrap ${JetBrainFont.className} selection:bg-emerald-700`}>{item.content}</pre>
                </div>
            </div >
        ))}</>)
    }
    ...
    function Five() {
        return (
            <div className='mt-20'>
                <h1 className={`${TauriFont.className} text-2xl font-bold mb-3 uppercase`}>Youtube Link:</h1>
                {ytvideo.map((link, index) => (
                    <div key={index} className='flex gap-2 justify-start h-8 items-center'>
                        <span>🧷</span>
                        <Link href={link.url} target='_blank' className='text-blue-500'>{link.url}</Link>
                    </div>
                ))
                }
            </div >
    ...
    return (
        <div className={`mx-10 ${InterFont.className}`}>
            <Toaster duration={2500} visibleToasts={3} />
            <First />
            <Two />
            <Three />
            <Four />
